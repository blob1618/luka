...
class CategoryMovementTotal:
    category_name: str
    amount: Decimal


@dataclass
class MovementChartQueryResult:
    status: str
    message: str
    categories: list[CategoryMovementTotal] = field(default_factory=list)
...
class FinanceService:
    ...
    @classmethod
    def aggregate_movements_by_category(
        cls,
        user_id: Any,
        *,
        ...
        """Aggregate active movements for one user without truncating the result."""
        try:
            parsed_user_id = UUID(str(user_id))
        except (ValueError, TypeError, AttributeError):
            return MovementChartQueryResult("user_not_found", "Usuario no especificado")

        normalized_type = str(movement_type or "").strip().lower()
        if normalized_type not in cls.VALID_MOVEMENT_TYPES:
            return MovementChartQueryResult("invalid_filters", "Tipo de movimiento inválido")
        if start_date > end_date:
            return MovementChartQueryResult("invalid_filters", "Período inválido")

        normalized_currency = None
        if currency is not None:
            normalized_currency = str(currency).strip().upper()
            if not normalized_currency or len(normalized_currency) != 3:
                return MovementChartQueryResult("invalid_filters", "Moneda inválida")
        ...
        try:
            if session.query(Usuario.id).filter(Usuario.id == parsed_user_id).first() is None:
                return MovementChartQueryResult("user_not_found", "Usuario no encontrado")

            category_label = func.coalesce(Categoria.nombre, "Sin categoría")
            query = (
                session.query(
                    category_label.label("category_name"),
                    MovimientoFinanciero.moneda.label("currency"),
                ...
                .filter(MovimientoFinanciero.usuario_id == parsed_user_id)
                .filter(MovimientoFinanciero.anulado_en.is_(None))
                .filter(MovimientoFinanciero.tipo == normalized_type)
                .filter(MovimientoFinanciero.fecha_movimiento >= start_date)
                .filter(MovimientoFinanciero.fecha_movimiento <= end_date)
            )
            if normalized_currency is not None:
                query = query.filter(MovimientoFinanciero.moneda == normalized_currency)

            rows = query.group_by(category_label, MovimientoFinanciero.moneda).all()
            currencies = sorted({str(row.currency).upper() for row in rows})
            if normalized_currency is None and len(currencies) > 1:
                return MovementChartQueryResult(
                    "needs_currency",
                    "Hay movimientos en más de una moneda",
                    available_currencies=currencies,
                )

            selected_currency = normalized_currency or (currencies[0] if currencies else None)
            totals: dict[str, Decimal] = {}
            for row in rows:
                if selected_currency is not None and str(row.currency).upper() != selected_currency:
                    continue
                name = str(row.category_name or "Sin categoría").strip() or "Sin categoría"
                totals[name] = totals.get(name, Decimal("0")) + Decimal(str(row.total))

            categories = [
                CategoryMovementTotal(category_name=name, amount=amount)
            ...
            print(f"[MOVEMENT_CHART_QUERY] Error: {type(exc).__name__}: {exc}")
            return MovementChartQueryResult("error", "Error interno al consultar movimientos")
        finally:
            if close_session:
                session.close()
